...
def save_nifti(all_slices, path, file_name, is_gt, header, image_type):
    ...
    if image_type == constant.CINE_TYPE:
        file_suffix = '_rm_gt.nii.gz' if is_gt else "_rm.nii.gz"
    elif image_type == constant.DE_TYPE:
        file_suffix = '_rm.nii.gz'
...
cine_target_slices = [0,7]
de_target_slices = [0,7,8]
cine_gt_target_slices = [0,7]
de_gt_target_slices = [0,7,8]
...
save_nifti(de_gt_slices, de_gt_final_path, "Case_P" + str(de_number), constant.IS_GT, de_gt.header, constant.DE_TYPE)
